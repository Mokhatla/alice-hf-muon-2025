...
std::string createSubdirectory(double centerOfMassEnergy, const std::string& pdfSet) {
    ...
    std::string subdirectory = "data/" + std::to_string(static_cast<int>(centerOfMassEnergy)) + "/" + pdfSet;
...
void *handle(void *ptr) {
    ...
    pythia.readString("PDF:pSet = 2");    // option 2 = CTEQ66
    ...
    std::string subdirectory = "RAW_ROOT_FILES";
    std::filesystem::create_directories("data/" + subdirectory);

    // Generate file paths within the subdirectory
    std::string metadata_file_path = "data/" + subdirectory + "/Simulation1_%ld_metadata.txt";
    std::string root_file_path = "data/" + subdirectory + "/Simulation1Results_%ld.root";
    ...
    int nEvents = 20000000; // Example initialization, set as needed
...
int main(int argc, char* argv[]) {
    ...
    const int numThreads = 10; // Number of threads to use
