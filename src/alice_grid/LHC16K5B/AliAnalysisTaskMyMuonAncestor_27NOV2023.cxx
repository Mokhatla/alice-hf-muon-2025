    ...
    */
    ...
    if(nTracksMC > 0) {
        for ( Int_t iTrack = 0; iTrack < nTracksMC; iTrack++ )
        {
            ...
            //AliVParticle *mcPart = dynamic_cast<AliVParticle*>(fMCEvent->GetTrack(iTrack));
            //if (!mcPart) {
            ...
            if(mcParticle) {
                ...
                if ( TMath::Abs(recoPdg) == 13 ) {
                    ...
                    while ( imother >= 0 ) {
                        ...
                        if ( isPrimary ) {
                            if (absPdg == 211) fPionMuPt->Fill(fTrack->Pt());
                            if (absPdg == 321) fKaonMuPt->Fill(fTrack->Pt());
                            
                            Int_t mpdg = absPdg%100000;
                            
                            if ( mpdg >= 100 && mpdg < 10000 ) {
                                Int_t flv  = Int_t ( mpdg / TMath::Power(10, Int_t(TMath::Log10(mpdg) )));
                                if ( flv < 4 ) fLightHadronMuPt->Fill(fTrack->Pt());
            ...
            if(fTrack->IsMuonTrack()) fHistMuPtWMTrig->Fill(fTrack->Pt());
            ...
            // if(!(ancs->IsMuon(fTrack,fMCEvent))) continue;
                ...
                if ( TMath::Abs(recoPdg) == 13 ) {
                    ...
                    while ( imother >= 0 ) {
                        ...
                        if ( isPrimary ) {
                            ...
                            if ( mpdg >= 100 && mpdg < 10000 ) {
                                Int_t flv  = Int_t ( mpdg / TMath::Power(10, Int_t(TMath::Log10(mpdg) )));
                                if ( flv < 4 ) fLightHadronMuPtWpDCA->Fill(fTrack->Pt());
                                //else if ( flv >= 6 ) continue;
                                //else {
                                    //TParticlePDG* partPdg = TDatabasePDG::Instance()->GetParticle(part->PdgCode());
                                    //if ( partPdg && ! partPdg->AntiParticle() ) SETBIT(fMask,kHasQuarkoniumParent);
            ...
            //AliAODTrack* track = (AliAODTrack*)fInputEvent->GetTrack(iTrack);   
            //if (!track)  continue;
            //muonMC = new AliMuonInfoStoreMC(track,mcEvent,fMuonTrackCuts->IsSelected(track),kFALSE);
            
            //if(TMath::Abs(track->PdgCode()) == 13) {
            //    fHMuonSourcePtpDCAWAcc->Fill(double(muonMC->Source())+0.5, track->Pt(), pDCA);
            //}
            // if isMuon 
            /*
            if(TriggerSelected_CMSL7){
                
                if(TMath::Abs(fTrack->PdgCode()) == 13) fHistMu->Fill(3);
                fHistPt->Fill(fTrack->Pt());
                fHistEta->Fill(fTrack->Eta());
                fHistRap->Fill(fTrack->Y());  
