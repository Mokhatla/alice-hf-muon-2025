...
void AliAnalysisTaskMyMuonAncestor::UserCreateOutputObjects()
{
    ...
    Int_t sigmaGen = 0.0;
    Int_t totalAcceptedCount = 0;
    Int_t sigmaGenerated = 0.0;

    // Set sigmaGen and totalAccepted only once
    if (fAOD) {
        fMCHeader = (AliAODMCHeader*)fAOD->FindListObject(AliAODMCHeader::StdBranchName());
        if (fMCHeader) {
            sigmaGen = fMCHeader->GetCrossSection(); // Set once
            totalAccepted = fMCHeader->GetTrials();  // Set once
        }
    }

    // Create metadata tree
    metadataTree = new TTree("metadataTree", "Simulation Metadata");
    metadataTree->Branch("totalAcceptedCount", &totalAcceptedCount, "totalAcceptedCount/L");
    metadataTree->Branch("sigmaGenerated", &sigmaGenerated, "sigmaGenerated/D");
    ...
    fOutputList->Add(sigmaGen);
    fOutputList->Add(totalAccepted);
...
void AliAnalysisTaskMyMuonAncestor::UserExec(Option_t *)
{
    ...
    */
    ...
    if (fMCHeader) {
        ...
        sigmaGenerated = fMCHeader->GetCrossSection();
        totalAccepted = fMCHeader->GetTrials(); // Replace with actual method
        totalAcceptedCount = fMCHeader->GetTrials(); // Replace with actual method
    }
